...
int main() {
    ...
    // A literal is a piece of data that is directly reperesented in code
    // without going through some other variable stored in memory.

    int int_var {55};   // value 55 will be stored in binary itself. it will not be stored in RAM.


    // Literal types: u and l combinations for unsigned and long.
    unsigned char unsigned_char {54u}; // 555u would fail because of narrowing

    //2 Bytes
    short short_var {-32768}; // No special literal moifier for short
    short int short_int {455}; // No special literal modifier for short
    signed short signed_short {122}; // No special literal modifier for short
    signed short int signed_short_int {-456}; // No special literal modifier for short
    unsigned short int unsigned_short_int {5678U};

    // 4 Bytes
    const int const_int_var {55};
    ...
    std::cout << "The prize is : " << prize << std::endl; // 1500000
    ...
    std::cout << "Black color is : " << std::dec << black_color << std::endl; // 16777215
